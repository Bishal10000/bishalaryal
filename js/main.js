document.addEventListener('DOMContentLoaded', () => {
    ...
    let cards = Array.from(document.querySelectorAll('[data-article-card]'));
    ...
    const sortCards = () => {
        const container = document.querySelector('.articles-grid__list');
        if (!container) return;

        const items = Array.from(container.querySelectorAll('[data-article-card]'));
        items.sort((a, b) => {
    ...
    const applyFilters = () => {
        // refresh cards list (order may change after sorting)
        cards = Array.from(document.querySelectorAll('[data-article-card]'));

        // perform sorting before computing visibility/order
        sortCards();
